...
import { SpinnerSettingsController } from './SpinnerSettingsController';
...
import { Audiotrack, Camera, Folder, FormatSize, Movie, PictureInPicture, SettingsEthernet, SwitchVideo, ZoomIn } from '../../../elements/Icons';
...
import { ZoomSettingsTab } from '../../base/settings/tabs/zoom/ZoomSettingsTab';

/**
 *
 */
@provider(SpinnerSettingsController, SpinnerSettingsStore)
@observer
/** */
export class SpinnerSettingsComponent extends BaseRoutedComponent<IMainRoutedProps> {
	@inject
	private readonly controller!: SpinnerSettingsController;
			...
			<div>
				<OneLine className='padding-left-12px box-sizing-border-box'>
					...
					<RightContainer className='padding-right-12px'>
						<ButtonGroup>
							...
							>
								{SettingsLocalization.common.resetButton(language)}
							</Button>
							<Button
								color='secondary'
				...
				>
					...
					>
						...
						/>
						<Tab
							label={SettingsLocalization.zoomTab.title(language)}
							index={5}
							icon={<ZoomIn/>}
						/>
						<Tab
					...
					>
						<ZoomSettingsTab
							...
							language={language}
							onChange={this.controller.onZoomSettingsChange}
						/>
					</TabPanel>
					<TabPanel
						value={tabSelected}
	...
	/** */
	private readonly onResetClick = (_event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
